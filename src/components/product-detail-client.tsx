...
import { ProductUpsellOffer } from "@/components/product-upsell-offer";
import { ProductVariantPicker } from "@/components/product-variant-picker";
import { copy } from "@/data/brand";
import { getProductReviews } from "@/data/product-reviews";
import { getProductUpsell } from "@/data/product-upsells";
...
import { getStockUrgencyMessage } from "@/lib/catalog/stock-urgency";
...
export function ProductDetailClient({ product }: ProductDetailClientProps) {
  ...
  const stockUrgency = getStockUrgencyMessage(product);
  const reviews = getProductReviews(product.slug);
  const upsell = getProductUpsell(product);
    ...
    <>
      <div className="grid gap-5 lg:grid-cols-2 lg:gap-14">
        ...
        <div className="flex flex-col">
          ...
          {stockUrgency && (
            <p className="-mt-2 text-xs font-medium text-[#b45309] sm:text-sm">
              {stockUrgency}
            </p>
          )}
          ...
          <div className="mt-4 hidden gap-3 pt-1 sm:flex sm:flex-wrap lg:pt-2">
            <AddToCartButton
              product={display}
      ...
      {upsell && (
        <ProductUpsellOffer
          product={product}
          accessory={upsell.accessory}
          offer={upsell.offer}
        />
      )}

      <ProductReviewsSection reviews={reviews} productName={product.name} />
