...
import { getVisibleBestsellersByStore, getVisibleProducts, sortProductsByPriceAsc } from "@/lib/catalog/visible-products";
...
export default async function HomePage() {
  ...
  const shopMorePicks = sortProductsByPriceAsc(visibleProducts).slice(0, 18);
  ...
  const storeBestsellers = await Promise.all(
    storeList.map(async (store) => ({
      store,
      products: await getVisibleBestsellersByStore(store.id, 6),
    })),
  );
      ...
      <section className="mt-14">
        <div className="mb-6 flex items-end justify-between gap-4">
          <div>
            ...
            <p className="section-subtitle mt-1">{copy.shopMoreSub}</p>
          </div>
          <Link
            href="/products"
